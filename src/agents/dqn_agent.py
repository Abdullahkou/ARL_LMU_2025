...
class DQNAgent(IAgent):
    def __init__(
        ...
        hp = DQNHyperParams()
        # overwrite default
        for fld in hp.__dataclass_fields__.keys():
            if hasattr(config, fld):
                setattr(hp, fld, getattr(config, fld))
        self.hp = hp
    ...
    def learn(
        ...
        try:
            while self.global_step < total_steps:
                    ...
                    for _ in range(self.hp.gradient_steps):
                        ...
                        if train_logger is not None:
                            if hasattr(train_logger, "log_dict"):
                                train_logger.log_dict(logs, step=self.global_step)
                            elif hasattr(train_logger, "log"):
                                for k, v in logs.items():
                                    try:
                                        train_logger.log(k, v, step=self.global_step)
                                    except Exception:
                                        pass
                ...
                #  evaluation --OPTIONAL--
                if eval_fn is not None and eval_schedule is not None:
                    if (self.global_step - last_eval) >= int(eval_schedule):
