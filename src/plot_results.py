...
def plot_results(
    ...
    for agent_name, (mean_file, std_file) in agents_to_plot.items():
        mean_df = pd.read_csv(mean_file, index_col=0, dtype=str)  # read as str to avoid errors on parse
        mean_df = mean_df.apply(pd.to_numeric, errors='coerce')  # non parsable values coerced to nan
