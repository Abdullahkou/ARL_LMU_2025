...
def eval_policy(env_id, model, seeds, save_dir, episodes_per_seed=5):
    ...
    for s in seeds:
        ...
        ep_logger = EpisodeLogger(intermediate_results_file=file)
    ...
    save_rolling_avg(
        mean_df, f"{save_dir}/{SMA_MEAN_FILE}", window_size=3, min_periods=1
    )
    ...
    save_rolling_avg(std_df, f"{save_dir}/{SMA_STD_FILE}", window_size=3, min_periods=1)
...
def main():
    ...
    )  # in echt: mehrere kompatible Mitglieder
    env.close()
